...
### 4-5. 외부 서비스 링크 추가 (가장 흔한 경우 — 허브의 본래 역할)

따로 배포된 서비스(Cloudflare·Vercel·Railway 등) 또는 **외부 팀원이 만든 서비스**를 허브에 올릴 때다. 코드를 허브에 넣지 않고 **링크 카드 한 장만** 추가한다.

**필요한 정보 4가지** (이것만 있으면 끝):
1. **접속 URL** — 예: `https://teammate-app.pages.dev`
2. **서비스 이름** — 카드 제목
3. **한 줄 소개** — 카드 설명
4. (선택) 표시할 도메인·분량 메모 — 카드 하단

`index.html`의 `<section class="cards">` 안에 아래 스니펫을 붙여넣고 내용만 바꾼다. **내부 카드와 다른 점**: `href`가 외부 URL이고, `target="_blank" rel="noopener noreferrer"`이며, 제목에 `↗` 화살표가 붙는다.

```html
<a class="card card-page" href="https://여기에-접속-URL" target="_blank" rel="noopener noreferrer"
   data-platform="naver" data-type="page"
   data-tags="검색용 한국어 키워드 공백으로 구분">
  <div class="card-tags">
    <span class="tag tag-naver">#네이버</span>
    <span class="tag tag-page">#서비스</span>
  </div>
  <h2 class="card-title">서비스 이름 <span class="ext-arrow" aria-hidden="true">↗</span></h2>
  <p class="card-desc">
    서비스에 대한 짧은 설명. 2-3줄 이내.
  </p>
  <div class="card-footer">
    <span class="card-meta">표시할-도메인.com</span>
    <span class="card-cta">열기 ↗</span>
  </div>
</a>
```

그 뒤는 §4-2의 7~8단계(로컬 테스트 → commit·push)와 동일하다. 외부 서비스가 **실제 배포돼 주소가 살아있어야** 한다는 점만 전제다(아직 배포 전이면, 만든 사람에게 배포부터 안내).
