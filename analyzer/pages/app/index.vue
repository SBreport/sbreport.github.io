...
<template>
  <div class="h-full flex flex-col gap-4">
    ...
    <!-- Block 1: 이용 현황 바 -->
    <div class="shrink-0 rounded-lg border border-gray-200 p-4 flex items-center gap-4">
    ...
    <!-- Block 4: 최근 분석한 키워드 -->
    <div class="shrink-0 rounded-lg border border-gray-200 p-4 flex flex-col gap-3">
      ...
      <div v-else class="flex flex-col divide-y divide-gray-100">
        <button
          ...
          :key="item.id"
          class="flex items-center gap-3 py-2 text-sm text-left hover:bg-gray-50 rounded px-1 transition-colors"
        ...
        >
          ...
          <span class="tabular-nums text-gray-600 shrink-0">
            {{ formatNumber(item.pc_volume) }} + {{ formatNumber(item.mobile_volume) }}
          </span>
          <span class="text-xs text-gray-400 shrink-0 w-16 text-right">{{ relativeTime(item.created_at) }}</span>
        </button>
      </div>
    </div>

    <!-- Block 5 + 6: 확장 프로그램 / 문의 -->
    <div class="shrink-0 grid grid-cols-2 gap-4">
