...
type DetailTab = 'reviews' | 'stats' | 'collections' | 'samples' | 'ai-diagnosis' | 'sprint' | 'blind-test'
...
const blindUsePlaceFilter = ref(false)
...
const blindResultPool = ref('')  // 특정 pool id 조회 (빈 문자열=전체)
...
async function createBlindPool() {
  ...
  try {
    ...
    if (blindUsePlaceFilter.value && selectedPlace.value) {
      body.place_row_id = selectedPlace.value.id
    }
...
async function fetchBlindResults() {
  blindResultStatus.value = 'loading'
  blindResultError.value = null
  blindResult.value = null
  try {
    const params = new URLSearchParams()
    if (blindResultPool.value.trim()) params.set('pool', blindResultPool.value.trim())
      ...
      <div class="flex-1 min-h-0 flex flex-col border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
        ...
        <template v-if="selectedPlace || activeTab === 'sprint'">
          ...
          <div class="shrink-0 flex border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-x-auto">
            ...
            <!-- 블라인드 평가 탭: researcher/admin만 -->
            <button
              v-if="authStore.isResearcher"
              class="px-4 py-2 text-xs font-medium border-b-2 transition-colors whitespace-nowrap"
              :class="activeTab === 'blind-test'
                ? 'border-primary-500 text-primary-600 dark:text-primary-400'
                : 'border-transparent text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200'"
              @click="activeTab = 'blind-test'"
            >
              블라인드 평가
            </button>
          </div>
          ...
          <!-- ══ 탭 7: 블라인드 평가 ════════════════════════════════════ -->
          <div v-if="authStore.isResearcher" v-show="activeTab === 'blind-test'" class="flex-1 min-h-0 overflow-y-auto p-4 flex flex-col gap-4">

            <!-- 안내 -->
            <div class="text-xs text-gray-500 dark:text-slate-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded px-3 py-2">
              평가 페이지: <code class="font-mono text-amber-700 dark:text-amber-400">/rate</code>
              &nbsp;—&nbsp;접근코드는 연구실에 별도 공유 (코드 자체는 여기 노출되지 않습니다)
            </div>

            <!-- ─ 풀 구성 카드 ─ -->
            <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-4 flex flex-col gap-3">
              <h3 class="text-sm font-semibold text-gray-800 dark:text-slate-100">풀 구성</h3>
              ...
              <label class="flex items-center gap-2 cursor-pointer">
                <input v-model="blindUsePlaceFilter" type="checkbox" class="rounded border-gray-300 dark:border-slate-600" />
                <span class="text-xs text-gray-600 dark:text-slate-300">
                  선택된 지점으로 필터
                  <span v-if="selectedPlace" class="text-gray-400 dark:text-slate-500">({{ selectedPlace.name ?? selectedPlace.place_id }})</span>
                  <span v-else class="text-gray-400 dark:text-slate-500">(좌측에서 지점 선택 필요)</span>
                </span>
              </label>
            ...
            <!-- ─ 결과 카드 ─ -->
            <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-4 flex flex-col gap-3">
              <h3 class="text-sm font-semibold text-gray-800 dark:text-slate-100">결과</h3>

              <!-- pool id 입력 (선택) -->
              <div class="flex gap-2 items-end">
                <div class="flex flex-col gap-1 flex-1">
                  <label class="text-xs text-gray-500 dark:text-slate-400">Pool ID (비워두면 전체)</label>
                  <input
                    v-model="blindResultPool"
                    type="text"
                    placeholder="pool-xxxxxxxx 또는 빈 칸"
                    class="w-full border border-gray-300 dark:border-slate-600 rounded px-2.5 py-1.5 text-sm bg-white dark:bg-slate-700 text-gray-800 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500"
                  />
                </div>
                <UButton
                  label="불러오기"
                  ...
                  variant="outline"
                  icon="i-heroicons-arrow-down-tray"
                  :loading="blindResultStatus === 'loading'"
                  :disabled="blindResultStatus === 'loading'"
                  @click="fetchBlindResults"
                />
              </div>
              ...
              <template v-else-if="blindResultStatus === 'done' && blindResult">
                ...
                <div class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-slate-400">
                  ...
                  <span>Pool: <code class="font-mono text-[11px] text-gray-600 dark:text-slate-300">{{ blindResult.pool }}</code></span>
                </div>
              ...
              <!-- Idle -->
              <div v-else-if="blindResultStatus === 'idle'" class="text-xs text-gray-400 dark:text-slate-500 py-2">
                풀 ID를 입력하거나 비워두고 "불러오기"를 눌러 전체 결과를 조회합니다.
              </div>
            </div>
